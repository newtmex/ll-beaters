...
export default function useAccount(): AccountDetail {
  ...
  const [authenticate, { setAuthenticationToken, resetAuthenticationToken }] = useAuthenticate();
  const [cookies, setCookie, removeCookie] = useCookies<string, COOKIES>([COOKIE_JWT_TOKEN]);
  ...
  const isLoggedInWhileReconnect = useMemo(() => isReconnecting && !!jwt, [isReconnecting, jwt]);
